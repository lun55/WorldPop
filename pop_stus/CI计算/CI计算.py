...
# ========== 主程序循环 ==========
# 验证函数
def validate_ci_calculation():
    # 创建一个测试数据集
    test_data = pd.DataFrame({
        'pop_exposed': [100, 200, 300, 400],
        'per_capita_gdp': [1000, 2000, 3000, 4000],  # 从穷到富
        'y_mid': [50, 60, 70, 80]  # 假设噪音水平
    })
    
    # 预期：如果富人区噪音更高，CI应为正
    # 如果穷人区噪音更高，CI应为负
    result = calculate_ci(test_data)
    print(f"测试结果: CI = {result['CI']:.4f}")
validate_ci_calculation()
...
for file in csv_files:
    ...
    try:
        results = df.groupby(gaul_cols).apply(lambda x: calculate_ci(x)).reset_index()
